...
export default function Login() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      saveToken(token);
      // setAuthToken was handled at startup in main.tsx; re-setting is OK:
      (await import("../lib/api")).setAuthToken(token);

      // optional: backend may not return user; so just navigate buyer by default
      nav("/buyer/create");
      ...
      <div className="bg-neutral-800 p-8 rounded-xl shadow-xl w-full max-w-md border border-neutral-700">
        ...
        <form onSubmit={submit} className="space-y-5">
          <div>
            ...
            <input className="w-full px-4 py-2 rounded bg-neutral-900 border border-neutral-700 text-neutral-100 placeholder-neutral-500 focus:ring-2 focus:ring-indigo-500"
              value={email} onChange={(e) => setEmail(e.target.value)} placeholder="buyer1@local" />
          </div>

          <div>
            <label className="block text-sm mb-1 text-neutral-300">Password</label>
            <input className="w-full px-4 py-2 rounded bg-neutral-900 border border-neutral-700 text-neutral-100 placeholder-neutral-500 focus:ring-2 focus:ring-indigo-500"
              type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="pass123" />
          </div>

          <button disabled={busy} className={`w-full py-2 rounded text-white ${busy ? "bg-indigo-400" : "bg-indigo-600 hover:bg-indigo-700"}`}>
            {busy ? "Logging in…" : "Login"}
          </button>
        </form>
        <p className="text-neutral-400 text-sm mt-4 text-center">Tip: use your Postman created user</p>
      </div>
    </main>
  );
}
